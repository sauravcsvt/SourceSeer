...
class source_pred(object):
    '''
    This class is responsible for calculating the source-topic relevance across 
    all locations for a future time point for which we are forecasting a rare disease
    outbreak
    '''
    ...
    def predict(self, result_queue, estimatedtime, estimatedlocation, sourcedict, 
                locationdict, worddict, estimatedword, totalword, totalprovince, 
                source_list, totaldictionary):
        ...
        for location in totalprovince:
            for source in source_list:
                ...
                cosine_sim_topic = self.similarity(topicwordcontent, singleworddict, numtopic, worddict, totalword)
                for k in xrange(numtopic):
                    tsldict[(k, source, location, timelength)] = cosine_sim_topic[0][k]
        result_queue.put(tsldict)
        return

    def similarity(self, topicwordcontent, word_count_vec, numtopic, worddict, totalword):

        cosine_sim_arr = np.zeros((1, numtopic))
        for k in xrange(numtopic):
            sumxx = 0
            sumxy = 0
            sumyy = 0
            for word in totalword:
                ...
                sumyy += y * y
                sumxy += x * y
            cosine_sim_arr[0][k] = sumxy / math.sqrt(sumxx * sumyy)
...
def parse_args():
    ...
    ap = argparse.ArgumentParser('Source-Topic Relevance calculation code')

    # Required Program arguments

    ap.add_argument("-s", "--start", type=str, required=True,
                    help="Epiweek Start Date for which source-topic relevances are to be calculated across all locations")
    ap.add_argument("-i", "--inputfolder", type=str, required=True, 
                    help="Input Folder containing the output of the tensor decomposition")
    arg = ap.parse_args()
    return arg
...
def main():
    ...
    datapath = _arg.inputfolder 
    os.chdir(datapath)
    src_pred = source_pred()
    ...
    source_partition = src_pred.partition(totalsourcelist, 80)

    # implementing multiprocessing to run the code faster

    result_queue = multiprocessing.Queue()

    jobs = [multiprocessing.Process(target=src_pred.predict, args=(result_queue, estimatedtime, 
                                                                   estimatedlocation, sourcedict, 
                                                                   locationdict, worddict, estimatedword, 
                                                                   totalwordlist, truncated_loc, source_list, totalmetricdict))
            for source_list in source_partition]
